...
// Real Nanoblock product/packaging images from Merlinsbricks (Diamond Blocks) CDN.
// Pattern: https://cdn.merlinsbricks.com/images/{CODE}/main/{CODE}.webp
// If a set isn't in their catalog the browser falls back to the PokéAPI artwork (onError in SetCard).
const nbImage = (code: string) =>
  `https://cdn.merlinsbricks.com/images/${code}/main/${code}.webp`

// PokéAPI official artwork — used for:
//   • RS series (NBPM-R01–R13): Merlinsbricks not yet stocking these
//   • 20th Anniversary Monotone (NBPM-014–017): limited JP-only sets
//   • Pokémon Quest editions (NBPM-037–042): Merlinsbricks 404
//   • Any other sets where nbImage() 404s (onError in SetCard falls back automatically)
...
export const CATALOG: NanoblockSet[] = [
  ...
  // Japan-only limited sets — Merlinsbricks doesn't stock them; falls back to PokéAPI artwork
  ...
  // Cube-art style sets — Merlinsbricks doesn't stock them; falls back to PokéAPI artwork
  ...
  { id: 'NBPM-054', pokemonName: 'Celebi',             pokemonNumber: 251, generation: 2, setCode: 'NBPM-054', imageUrl: nbImage('NBPM-054') },
  { id: 'NBPM-055', pokemonName: 'Gyarados (Crystal)', pokemonNumber: 130, generation: 1, setCode: 'NBPM-055', imageUrl: nbImage('NBPM-055') },
  { id: 'NBPM-056', pokemonName: 'Lapras (Crystal)',   pokemonNumber: 131, generation: 1, setCode: 'NBPM-056', imageUrl: nbImage('NBPM-056') },
  ...
  { id: 'NBPM-081', pokemonName: 'Pikachu (Lunar New Year)', pokemonNumber: 25, generation: 1, setCode: 'NBPM-081', imageUrl: nbImage('NBPM-081') },
  ...
  // Merlinsbricks doesn't carry the RS series yet — using PokéAPI artwork as primary
